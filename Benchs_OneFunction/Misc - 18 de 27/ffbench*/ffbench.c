...
#define Im(x, y) fdata[2 + (faedge * (x) + (y)) * 2]
  ...
  for (i = 0; i < npasses; i++) {
    ...
    //static void fourn(data, nn, 2, isign) 
      ...
      for (idim = 2; idim >= 1; idim--) {
          ...
          if (i2 < i2rev) {
            for (i1 = i2; i1 <= i2 + ip1 - 2; i1 += 2) {
              for (i3 = i1; i3 <= ip3; i3 += ip2) {
                i3rev = i2rev + i3 - i2;
                //new swap
                double tempr = fdata[i3];                                                                 \
                fdata[i3] = fdata[i3rev];                                                                   \
                fdata[i3rev] = tempr;

                tempr = fdata[i3 + 1];
                fdata[i3 + 1] = fdata[i3rev + 1];
                fdata[i3rev + 1] = tempr;
        ...
        while (ifp1 < ip2) {
          ifp2 = ifp1 << 1;
          theta = 1 * 6.28318530717959 / (ifp2 / ip1);
          wtemp = sin(0.5 * theta);
          wpr = -2.0 * wtemp * wtemp;
          wpi = sin(theta);
          wr = 1.0;
          ...
          for (i3 = 1; i3 <= ifp1; i3 += ip1) {
            for (i1 = i3; i1 <= i3 + ip1 - 2; i1 += 2) {
              for (i2 = i1; i2 <= ip3; i2 += ifp2) {
                k1 = i2;
                k2 = k1 + ifp1;
                tempr = wr * fdata[k2] - wi * fdata[k2 + 1];
                tempi = wr * fdata[k2 + 1] + wi * fdata[k2];
                fdata[k2] = fdata[k1] - tempr;
                fdata[k2 + 1] = fdata[k1 + 1] - tempi;
                fdata[k1] += tempr;
                fdata[k1 + 1] += tempi;
              }
            }
