...
char *argv[];
{
  ...
  niter = 1000000;
	...
	for (paraxial = 0; paraxial <= 1; paraxial++) {
		...
		//trace_line(4, clear_aperture / 2.0) EXTERNO;
		int i;
		int line = 4;
		double ray_h = clear_aperture / 2.0;
		...
		for (i = 1; i <= current_surfaces; i++) {
			...
			to_index = s[i][2];
			if (to_index > 1.0) to_index = to_index + ((spectral_line[4] - spectral_line[line]) / (spectral_line[3] - spectral_line[6])) * ((s[i][2] - 1.0) / s[i][3]);
			
			//transit_surface() INTERNO
			...
			if (paraxial) {
				if (radius_of_curvature != 0.0) {
					...
					} else iang_sin = ((object_distance - radius_of_curvature) / radius_of_curvature) * axis_slope_angle;
